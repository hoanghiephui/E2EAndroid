...
public class RSA {
    ...
    public static String encryptWithStoredKey(Context mContext, String text) {
    ...
    public static String decryptWithStoredKey(Context mContext, String text) {
        String privateK = SecurePrefManager.with(mContext)
                .get(com.e2e.message.data.Constants.RSA_PRIVATE_KEY)
                .defaultValue("unknown")
                .go();
        try {
            String strippedKey = Crypto.stripPrivateKeyHeaders(privateK);
